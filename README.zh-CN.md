...
- **改写 HTML/JS**：只在 `200` 响应且 `content-type` 为 HTML / JavaScript / Web App Manifest 时执行。`/assets/`、`/api`、`/plugins/`、`/dsh-market/`、`/_dsh/`、manifest、favicon 等根绝对引用会被统一加上 base 前缀；base 插在开引号之后，双引号、单引号、反引号三种形式都覆盖，保证改写后的字符串仍是合法 JSON/HTML/JS。更具体的路径先于通用前缀改写，因此不会出现二次加前缀。
- **其余响应零缓冲透传**：二进制、SSE、重定向、错误响应等直接管道转发，不整包缓冲进内存。
...
- **优雅退出**：收到 `SIGINT`/`SIGTERM` 时关闭监听并结束由代理拉起的 `dsh web`，不留孤儿进程。
